...
def generate_comparative_pdf(data: dict[str, Any]) -> BytesIO:
    ...
    # Tabla: Producto | Fuente1 | ... | Tienda (mín.) | Precio (IVA incl.)
    headers = ["Producto / Servicio"] + sources + ["Tienda (mín.)", f"Precio ({currency}) IVA incl."]
    table_data = [headers]
    ...
    total_row = [Paragraph("<b>TOTAL (IVA incluido)</b>", styles["Normal"])] + [""] * len(sources) + [""] + [f"{total:.2f}"]
